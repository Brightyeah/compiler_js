...
int parser_body(struct list *list_parser) {
	...
	if (pnum == tok_var) {
		...
		if (parser_prev == tok_var || parser_prev == 40 || parser_prev == 43 || parser_prev == 44 || parser_prev == 45 
			  || parser_prev == tok_function || parser_prev == 59 || parser_prev == 60 || parser_prev == 61 
		...
		if (parser_prev == tok_if || parser_prev == tok_id || parser_prev == tok_else || parser_prev == tok_while
		...
		if (parser_prev == tok_id || parser_prev == tok_num_int || parser_prev == tok_num_double ||
			parser_prev == tok_string || parser_prev == 41 || parser_prev == 93) {}// ) ]
		else error_parser = 1;
